 ...
 * @param param
 */
void book_mng_action(GSimpleAction *action, GtkApplication *app,
                     gpointer param) {
  GtkWindow *win = param;
  //
  //
 ...
 * @param win
 */
void app_startup(GApplication *application, GtkWidget *win) {
  GtkApplication *app = GTK_APPLICATION(application);

  GSimpleAction *act_quit = g_simple_action_new("quit", NULL);
  GSimpleAction *act_book_mng =
      g_simple_action_new("book-mng", NULL); // Action Book Manager

  g_action_map_add_action(G_ACTION_MAP(app), G_ACTION(act_quit));
  g_action_map_add_action(G_ACTION_MAP(app),
                          G_ACTION(act_book_mng)); // Add new window action

  // CORRECTED: Connect act_file to file_activated and pass 'application' as
  // user_data
  g_signal_connect(act_quit, "activate", G_CALLBACK(quit_activated),
                   application);
  g_signal_connect(act_book_mng, "activate", G_CALLBACK(book_mng_action),
                   win); // Connect new window action
  ...
  GMenuItem *menu_item_book_mng =
      g_menu_item_new("Manager Books", "app.book-mng");
...
void app_activate(GApplication *application) {
  ...
  gtk_picture_set_keep_aspect_ratio(GTK_PICTURE(picture),
