...
/**
 * @brief on_alert_response
 * @param source_object
 * @param result
 * @param user_data
 **/
static void
on_alert_response(GObject *source_object,
                    GAsyncResult *result,
                      gpointer user_data)
{
  g_autoptr(GError) error = NULL;
  // Cast the source_object back to GtkAlertDialog
  GtkAlertDialog *alert_dialog = GTK_ALERT_DIALOG(source_object);

  // Get the response ID (index of the clicked button)
  int response_id =
      gtk_alert_dialog_choose_finish(alert_dialog, result, &error);

  if (error) {
    g_printerr("Error showing alert dialog: %s\n", error->message);
    return;
  }

  // Since there's only one button ("OK") at index 0, we can simplify the check.
  // However, keeping the general approach for robustness.
  const gchar *const *buttons = gtk_alert_dialog_get_buttons(alert_dialog);
  int num_buttons = 0;
  if (buttons) {
    while (buttons[num_buttons] != NULL) {
      num_buttons++;
    }
  }

  if (response_id >= 0 && response_id < num_buttons) {
    g_print("Alert dialog closed. You clicked: %s\n", buttons[response_id]);
  } else {
    g_print("Alert dialog closed. Unknown response ID: %d\n", response_id);
  }

  // The alert_dialog is a transient dialog, it will be destroyed automatically
  // when it's closed, so no need for
  // gtk_widget_destroy(GTK_WIDGET(alert_dialog));
}

/**
 * @brief on_show_alert_clicked
 * @param button
 * @param user_data
 */
void
                     ...
                     gpointer user_data)
{
  ...
  // Create a GtkAlertDialog with just the message
  g_autoptr(GtkAlertDialog) alert =
      gtk_alert_dialog_new("Operation Completed Successfully!",
                           NULL // No format arguments for the message
      );

  // Set the detail text (optional, can be removed if not needed)
  gtk_alert_dialog_set_detail(alert, "Your action has been processed.");

  // Set the buttons with only an "OK" button
  gtk_alert_dialog_set_buttons(alert, (const gchar *const[]){"OK", NULL});

  // Show the alert dialog asynchronously
  gtk_alert_dialog_choose(alert, parent_window, NULL, on_alert_response, NULL);
}
